"""
This script is used to fine-tune a RegNet model resolution on the custom dataset.
"""
...
def transform(batch, resolution=224):
    ...
    batch : dict
        The example batch.
    resolution : int
        The resolution to use. Defaults to 224.
...
def main():
    ...
    model_list = ["facebook/regnet-y-320-seer-in1k"]
    ...
    for model_name_or_path in model_list:
        ...
        prepared_ds_train = dataset_train.with_transform(
            lambda batch: transform(batch, resolution=image_res)
        )
        prepared_ds_val = dataset_val.with_transform(
            lambda batch: transform(batch, resolution=image_res)
        )
